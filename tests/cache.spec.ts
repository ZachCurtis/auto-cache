import Cache, { SectionedCache } from '../index'
...
describe('cache', function () {
    ...
    context('SectionCache', function () {
        it('should save and load a value in a section generically', async function () {
            await SectionedCache.bindMissHandler('users', 15000, async function (userKey: any) {
                switch (userKey) {
                    case 1:
                        return 'user id 1'
            ...
            setTimeout(async () => {
                let user0 = await SectionedCache.get('users', 1)
                assert.strictEqual(user0, 'user id 1')

                setTimeout(async () => {

                    let user1 = await SectionedCache.get('users', 2)
